...
#
np.random.seed(0)
X = np.random.random(size=3000)
X = X.reshape((30,100))
w1 = np.random.random(size=(1, 45)) * 10.0
w2 = np.random.random(size=(1, 55)) * 0.01
...
def test_regression ():
    model = gpmodel.LassoGPModel(gpkernel.LinearKernel(), gamma=1.0)
    model.fit(X_df, Y)
    print(model.gamma)
    print(model.hypers)
    print(len(model.X_seqs.columns))
    print(model.ML)
    print(model.score(X_df, Y, 'R'))
    return
